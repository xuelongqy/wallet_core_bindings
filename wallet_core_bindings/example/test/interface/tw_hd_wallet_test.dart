...
void main() {
  ...
  group(TWHDWallet, () {
    ...
    test('MasterPrivateKey', () {
      final wallet = TWHDWallet.createWithMnemonic(gWords);
      final key1 = wallet.getMasterKey(TWCurve.SECP256k1);
      final hexKey1 = key1.data;

      final key2 = wallet.getMasterKey(TWCurve.ED25519);
    ...
    test('Derive', () {
      final wallet =
          TWHDWallet.createWithMnemonic(gWords, passphrase: gPassphrase);
      final key0 = wallet.getKeyForCoin(TWCoinType.Ethereum);
    ...
    test('DeriveBitcoinNonextended', () {
      final wallet =
          TWHDWallet.createWithMnemonic(gWords, passphrase: gPassphrase);
      final key = wallet.getKeyForCoin(TWCoinType.Bitcoin);
    ...
    test('DeriveBitcoinExtended', () {
      ...
      final address = TWCoinType.Bitcoin.deriveAddressFromPublicKey(publicKey);
    ...
    test('DeriveAddressBitcoin', () {
      final wallet =
          TWHDWallet.createWithMnemonic(gWords, passphrase: gPassphrase);
      final address = wallet.getAddressForCoin(TWCoinType.Bitcoin);
      expect(address, 'bc1qumwjg8danv2vm29lp5swdux4r60ezptzz7ce85');
    });
    ...
    test('DeriveEthereum', () {
      final wallet =
          TWHDWallet.createWithMnemonic(gWords, passphrase: gPassphrase);
      final key = wallet.getKeyForCoin(TWCoinType.Ethereum);
      final key2 = wallet.getKeyForCoin(TWCoinType.SmartChain);

      final publicKey = key.getPublicKeySecp256k1(false);
      final publicKey2 = key2.getPublicKeySecp256k1(false);
      final publicKeyData = publicKey.data;

      final address = TWCoinType.Ethereum.deriveAddressFromPublicKey(publicKey);
      final address2 =
          TWCoinType.SmartChain.deriveAddressFromPublicKey(publicKey2);
    ...
    test('DeriveCosmos', () {
      ...
      final privateKey = wallet.getKeyForCoin(TWCoinType.Cosmos);
    ...
    test('DeriveNimiq', () {
      final wallet =
          TWHDWallet.createWithMnemonic(gWords, passphrase: gPassphrase);
      final key = wallet.getKeyForCoin(TWCoinType.Nimiq);
    ...
    test('DeriveTezos', () {
      final wallet =
          TWHDWallet.createWithMnemonic(gWords, passphrase: gPassphrase);
      final key = wallet.getKeyForCoin(TWCoinType.Tezos);
      final publicKey = key.getPublicKeyEd25519();
      final publicKeyData = publicKey.data;
    ...
    test('DeriveDoge', () {
      final wallet =
          TWHDWallet.createWithMnemonic(gWords, passphrase: gPassphrase);
      final key = wallet.getKeyForCoin(TWCoinType.Dogecoin);
      final publicKey = key.getPublicKeySecp256k1(true);
      final publicKeyData = publicKey.data;
      ...
      final xprv = wallet.getExtendedPrivateKey(
        ...
        coin: TWCoinType.Dogecoin,
        version: TWHDVersion.DGPV,
      );
      expect(xprv,
    ...
    test('DeriveZilliqa', () {
      final wallet =
          TWHDWallet.createWithMnemonic(gWords, passphrase: gPassphrase);
      final key = wallet.getKeyForCoin(TWCoinType.Zilliqa);
      final publicKey = key.getPublicKeySecp256k1(true);
      final publicKeyData = publicKey.data;
    ...
    test('DeriveAionPrivateKey', () {
      ...
      final privateKey = wallet.getKeyForCoin(TWCoinType.Aion);
      final privateKeyData = privateKey.data;
    ...
    test('DeriveFIO', () {
      ...
      final privateKey = wallet.getKeyForCoin(TWCoinType.FIO);
      final privateKeyData = privateKey.data;
    ...
    test('DeriveAlgorand', () {
      final wallet =
          TWHDWallet.createWithMnemonic(gWords, passphrase: gPassphrase);
      final privateKey = wallet.getKeyForCoin(TWCoinType.Algorand);
      final privateKeyData = privateKey.data;
    ...
    test('DeriveMultiversX', () {
      final wallet =
          TWHDWallet.createWithMnemonic(gWords, passphrase: gPassphrase);
      final privateKey = wallet.getKeyForCoin(TWCoinType.MultiversX);
      final privateKeyData = privateKey.data;
      final address = TWCoinType.MultiversX.deriveAddress(privateKey);
    ...
    test('DeriveBinance', () {
      final wallet =
          TWHDWallet.createWithMnemonic(gWords, passphrase: gPassphrase);
      final key = wallet.getKeyForCoin(TWCoinType.Binance);
      final key2 = wallet.getKeyForCoin(TWCoinType.SmartChain);
    ...
    test('DeriveAvalancheCChain', () {
      final wallet =
          TWHDWallet.createWithMnemonic(gWords, passphrase: gPassphrase);
      final key = wallet.getKeyForCoin(TWCoinType.AvalancheCChain);
      final keyData = key.data;
    ...
    test('DeriveCardano', () {
      final wallet =
          TWHDWallet.createWithMnemonic(gWords, passphrase: gPassphrase);
      final privateKey = wallet.getKeyForCoin(TWCoinType.Cardano);
      final privateKeyData = privateKey.data;
      expect(privateKeyData.length, 192);
      final address = TWCoinType.Cardano.deriveAddress(privateKey);
    ...
    test('PublicKeyFromZ', () {
      ...
      final address4 = TWCoinType.Bitcoin.deriveAddressFromPublicKey(zpubAddr4);
    ...
    test('PublicKeyFromExtended_Ethereum', () {
      ...
      expect(xpubAddr.pointer != 0, true);
    ...
    test('GetKeyByCurve', () {
      ...
      final wallet = TWHDWallet.createWithMnemonic(gWords);
      final privateKey1 = wallet.getKeyByCurve(TWCurve.SECP256k1, derivPath);
      ...
      final privateKey2 = wallet.getKeyByCurve(TWCurve.NIST256p1, derivPath);
    ...
    test('FromMnemonicImmutableXMainnetFromSignature', () {
      ...
      final ethPrivateKey =
          wallet.getKey(TWCoinType.Ethereum, ethDerivationPath);
      ...
      final starkPubKey =
          starkPrivateKey.getPublicKeyByType(TWPublicKeyType.Starkex);
    ...
    test('Derive_XpubPub_vs_PrivPub', () {
      ...
      // -> privateKey -> publicKey
      {
        ...
        final address1 =
            TWSegwitAddress.createWithPublicKey(TWHRP.Bitcoin, publicKey1);
      ...
      }
      {
        ...
        final address2 =
            TWSegwitAddress.createWithPublicKey(TWHRP.Bitcoin, publicKey2);
          ...
          'zpub6rNUNtxSa9Gxvm4Bdxf1MPMwrvkzwDx6vP96Hkzw3jiQKdg3fhXBStxjn12YixQB8h88B3RMSRscRstf9AEVaYr3MAqVBEWBDuEJU4PGaT9');
      {
        ...
        expect(publicKey1.pointer != 0, true);
        final publicKey1Data = publicKey1.data;
        expectHex(publicKey1Data, expectedPublicKey1);
        final address1 =
            TWSegwitAddress.createWithPublicKey(TWHRP.Bitcoin, publicKey1);
      ...
      }
      {
        ...
        expect(publicKey2.pointer != 0, true);
        final publicKey2Data = publicKey2.data;
