...
void main() {
  ...
  group('TransactionPlan', () {
    test('OneTypical', () {
      ...
      final txPlan = Bitcoin.TransactionPlan.fromBuffer(
          TWAnySigner.plan(sigingInput.writeToBuffer(), TWCoinType.Bitcoin));
    ...
    test('OneSufficientLower170', () {
      ...
      const dustChange = 23;
      const actualFee = 147 + dustChange;
    ...
    test('OneFitsExactly', () {
      ...
      const expectedFee = 147 + dustChange;
    ...
    test('OneFitsExactlyHighFee', () {
      final utxos = buildTestUTXOs([100000]);
      const byteFee = 10;
      const dustChange = 270;
      // Change amount is too low (less than dust), so we just waste it as the transaction fee.
      const expectedFee = 1470 + dustChange;
    ...
    test('ThreeNoDust', () {
      ...
      var txPlan = Bitcoin.TransactionPlan.fromBuffer(
          TWAnySigner.plan(sigingInput.writeToBuffer(), TWCoinType.Bitcoin));
      ...
      txPlan = Bitcoin.TransactionPlan.fromBuffer(
          TWAnySigner.plan(sigingInput.writeToBuffer(), TWCoinType.Bitcoin));
    ...
    test('MaxAmountDoge', () {
      ...
      final sigingInput =
          buildSigningInput(2300000000, 100, utxos, true, TWCoinType.Dogecoin);
    ...
    test('AmountDecred', () {
      ...
      final sigingInput =
          buildSigningInput(10000000, 10, utxos, false, TWCoinType.Decred);
    ...
    test('ManyUtxosNonmax_400', () {
      ...
      final sigingInput = buildSigningInput(
          requestedAmount, byteFee, utxos, false, TWCoinType.Bitcoin);
    ...
    test('ManyUtxosNonmax_5000_simple', () {
      ...
      final utxos = buildTestUTXOs(values);
      final sigingInput = buildSigningInput(
          requestedAmount, byteFee, utxos, false, TWCoinType.Ravencoin);
    ...
    test('ManyUtxosMax_400', () {
      ...
      final utxos = buildTestUTXOs(values);
      final sigingInput = buildSigningInput(
          valueSum, byteFee, utxos, true, TWCoinType.Ravencoin);

      final txPlan = Bitcoin.TransactionPlan.fromBuffer(
          TWAnySigner.plan(sigingInput.writeToBuffer(), TWCoinType.Bitcoin));

      // all are selected, except a few smallest UTXOs are filtered out
      const dustLimit = byteFee * 148;
    ...
    test('OpReturn', () {
      const ownAddress = "bc1q7s0a2l4aguksehx8hf93hs9yggl6njxds6m02g";
