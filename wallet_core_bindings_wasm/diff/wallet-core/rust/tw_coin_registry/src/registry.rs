...
lazy_static! {
    static ref REGISTRY: Vec<CoinItem> = parse_registry_json();
}
...
pub fn get_coin_item(coin: CoinType) -> RegistryResult<&'static CoinItem> {
    registry_iter().find(|&item| item.coin_id == coin).ok_or(RegistryError::UnknownCoinType)
}
...
fn parse_registry_json() -> Vec<CoinItem> {
