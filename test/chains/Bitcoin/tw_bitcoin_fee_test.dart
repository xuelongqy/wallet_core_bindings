import 'package:test/test.dart';
import 'package:wallet_core_bindings/wallet_core_bindings.dart';

void main() {
  group('TWBitcoinFee', () {
    test('P2pkhFee', () {
      final satVb = '10';
      final data = TWData.createWithHexString(
              '')
          .bytes()!;
      final fee = TWBitcoinFee.calculateFee(data, satVb);
      expect(fee, (191 * 10).toString());
    });

    test('P2wpkhFee', () {
      final satVb = '12';
      final data = TWData.createWithHexString(
              '')
          .bytes()!;
      final fee = TWBitcoinFee.calculateFee(data, satVb);
      expect(fee, (189 * 12).toString());
    });

    // Metadata can be observed live on:
    // https://www.blockchain.com/explorer/transactions/btc/797d17d47ae66e598341f9dfdea020b04d4017dcf9cc33f0e51f7a6082171fb1
    //
    // Fee/VB   19.608 sat/vByte
    // Size     235 Bytes
    // Weight   610
    test('BRC20TransferCommitFee', () {
      final satVb = '19';
      final data = TWData.createWithHexString(
              '')
          .bytes()!;
      final fee = TWBitcoinFee.calculateFee(data, satVb);
      expect(fee, (153 * 19).toString());
    });
  });
}
