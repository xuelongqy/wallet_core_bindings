...
void main() {
  group('CardanoSigning', () {
    ...
    test('PlanForceFee', () {
      final requestedAmount = 6500000;
      final availableAmount = 8000000;
    ...
    test('SignNegative', () {
      {
        // plan with error
        final input = createSampleInput(7000000);
        final error = Common.SigningError.Error_invalid_memo;
        input.plan = Cardano.TransactionPlan(
          error: error,
        );
        final output = Cardano.SigningOutput.fromBuffer(
    ...
    test('SignMessageWithKey', () {
      ...
      final sampleMessageStr = "Hello world";
      final sampleMessage = TWData.createWithString(sampleMessageStr);

      final signature = privateKey.sign(
          sampleMessage.bytes()!, TWCurve.TWCurveED25519ExtendedCardano);

      final sampleRightSignature =
